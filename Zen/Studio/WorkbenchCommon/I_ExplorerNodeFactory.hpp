...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
#ifndef ZENSTUDIO_WORKBENCHCOMMON_I_EXPLORER_NODE_FACTORY_HPP_INCLUDED
#define ZENSTUDIO_WORKBENCHCOMMON_I_EXPLORER_NODE_FACTORY_HPP_INCLUDED
...
class WORKBENCHCOMMON_DLL_LINK I_ExplorerNodeFactory
...
public:
    ...
    virtual pProject_type createProject(I_ProjectExplorerController& _controller, pProjectDomainObject_type _pProjectDO, pExplorerNodeDomainObject_type _pExplorerNodeDO) = 0;

    /// Create an explorer node user data.
    virtual pUserData_type createNodeUserData(boost::uint64_t _explorerNodeId, const std::string& _nodeType, I_ExplorerNode& _parent) = 0;
    /// @}
