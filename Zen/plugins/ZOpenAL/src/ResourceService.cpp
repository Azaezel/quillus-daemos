...
//use after we've got everything else prepped for the dependencies libogg, libvorbis, and libvorbisfile
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
size_t 
...
ResourceService::loadResource(config_type& _config)
{
        ...
        iter != m_locationList.end(); iter++)
    {
        ...
        {
            data = LoadVorbisBuffer((ALbyte *)(fullPath.string().c_str()), &format, &size, &freq);
            internalMalloced = true;
        }
        else
        {
            data = alutLoadMemoryFromFile((ALbyte *)(fullPath.string().c_str()), &format, &size, &freq);
