...
const InputMap::KeyMapTable_type InputMap::sm_keyMapTable = boost::assign::map_list_of
//   Name                                   Code            State
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
InputMap::InputMap(wpInputService_type _pInputService)
:   m_pInputService(_pInputService)
,   m_isFocused(true)
,   m_pScriptObject(NULL)
{
    m_pMutex = Threading::MutexFactory::create();

    // TODO This should be a KeyInputMap
    m_eventConnection = m_pInputService->onKeyEvent.connect(boost::bind(&InputMap::onKeyPressed, this, _1));
}
...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
void
InputMap::mapKeyInput(const std::string& _name, const pAction_type _pAction)
{
    ...
    else
    {
        ...
        //throw Zen::Utility::runtime_exception("Zen::ZInput::InputMap::mapKeyInput() : Invalid key name.");
    }
}
...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
void
InputMap::onKeyPressed(pKeyEvent_type _pEvent)
{
    if( m_isFocused )
    {
        const KeyState* pState = dynamic_cast<const KeyState*>(&(_pEvent->getKeyState()));
        if( pState == NULL )
        {
            throw Zen::Utility::runtime_exception("Zen::ZInput::InputMap::onKeyPressed() : Invalid KeyState.");
        }

        KeyMap_type::iterator iter = m_keyMap.find(*pState);

        while( iter != m_keyMap.end() && iter->first == *pState )
        {
...
InputMap::getScriptObject()
{
    if (m_pScriptObject == NULL)
    {
        ...
        if(pModule.isValid())
        {
            m_pScriptObject = new Zen::Engine::Input::I_InputMap::ScriptObjectReference_type
