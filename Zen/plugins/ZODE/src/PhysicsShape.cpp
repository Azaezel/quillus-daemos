...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
bool
CollisionShape::initConeShape(float _radius, float _height)
{
    // TODO - set offsetMatrix (currently passing NULL, which centers the body at its origin)
    return attachBody(NewtonCreateCone((NewtonWorld*)m_pZone->getZonePtr(), _radius, _height, NULL));
}
...
//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
bool
CollisionShape::initCylinderShape(float _radius, float _height)
{
    // TODO - set offsetMatrix (currently passing NULL, which centers the body at its origin)
    return attachBody(NewtonCreateCylinder((NewtonWorld*)m_pZone->getZonePtr(), _radius, _height, NULL));
}

//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
bool
CollisionShape::initChamferCylinderShape(float _radius, float _height)
{
    // TODO - set offsetMatrix (currently passing NULL, which centers the body at its origin)
    return attachBody(NewtonCreateChamferCylinder((NewtonWorld*)m_pZone->getZonePtr(), _radius, _height, NULL));
}

//-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~
// GLOBAL
void
ZODESerializeFile(void* serializeHandle, const void* buffer, size_t size)
{
...
// GLOBAL
void
ZODEDeSerializeFile(void* serializeHandle, void* buffer, size_t size)
{
...
CollisionShape::initHeightFieldShapeFromRaw(std::string _filename, size_t _size, float _maxHeight, float _scaleXY, const Math::Matrix4& _transform, bool _bSerialize)
{
    ...
    if (_bSerialize)
    {
	    ...
	    NewtonTreeCollisionSerialize(collision, ZODESerializeFile, file);
...
CollisionShape::initHeightFieldShapeFromSerialization(std::string _filename, const Math::Matrix4& _transform)
{
    ...
    NewtonCollision* collision = NewtonCreateTreeCollisionFromSerialization((NewtonWorld*)m_pZone->getZonePtr(), NULL, ZODEDeSerializeFile, file);
	fclose (file);
